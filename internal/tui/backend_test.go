...
func TestSetShufflePlayModeKeepsRepeatSeparate(t *testing.T) {
	tests := []struct {
		name    string
		mode    string
		enabled bool
		...
		{name: "enable from repeat once", mode: "REPEAT_ONE", enabled: true, want: "REPEAT_ONE"},
...
func TestSetRepeatPlayModePreservesShuffle(t *testing.T) {
	tests := []struct {
		...
		{name: "shuffle on repeat once", mode: "SHUFFLE_NOREPEAT", repeat: "once", want: "REPEAT_ONE"},
...
func TestRepeatModeFromPlayModeTreatsShuffleAsRepeatAll(t *testing.T) {
