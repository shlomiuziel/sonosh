...
const repoBase = "https://github.com/shlomiuziel/sonosh";
const repoEditBase = `${repoBase}/edit/main/docs`;
const cname = readCname();
const siteBase = cname ? `https://${cname}` : "";

const productName = "sonoscli";
const productTagline = "Sonos in your terminal";
const productDescription =
  "A modern Go CLI to discover, group, and control Sonos speakers over your local network — built for terminals, scripts, automations, and coding agents.";
const brewInstall = "brew install steipete/tap/sonoscli";

// Sidebar layout. Section "Commands" is grouped at the bottom; rels under
// `commands/` other than the README still build their own pages but are not
// listed in the sidebar (the command index covers them).
const sections = [
  ["Start", ["index.md", "install.md", "quickstart.md"]],
  ["Concepts", ["architecture.md", "discovery.md", "spotify-and-smapi.md", "scenes.md", "output-and-scripting.md"]],
  ["Commands", ["commands/README.md"]],
  ["Reference", ["spec.md"]],
];

const buildExcludes = [/^improvements\.md$/, /^testing\.md$/];
const proseSections = new Set(["Start", "Concepts", "Reference"]);
...
// Pager order: nav order plus deep-linked command pages right after the index.
const orderedPages = (() => {
  const out = [];
  for (const section of nav) {
    for (const page of section.pages) {
      out.push(page);
      if (page.rel === "commands/README.md") {
        const cmdPages = pages
          .filter((p) => p.rel.startsWith("commands/") && p.rel !== "commands/README.md")
          .sort((a, b) => a.rel.localeCompare(b.rel));
        for (const cp of cmdPages) {
          out.push(cp);
          sectionByRel.set(cp.rel, "Commands");
        }
      }
    }
  }
...
function rewriteHref(href, currentRel) {
  ...
  if (!raw.endsWith(".md") && !raw.endsWith("/")) return href;
  const from = path.posix.dirname(currentRel);
  // Resolve trailing-slash refs (e.g. "commands/") to commands/README.md
  let resolveTarget = raw;
  if (raw.endsWith("/")) resolveTarget = `${raw}README.md`;
  const target = path.posix.normalize(path.posix.join(from, resolveTarget));
...
function homeHero(page) {
  ...
  const quickstartRel = pageMap.get("quickstart.md")?.outRel
    ...
    : "quickstart.html";
  const services = ["Discovery", "Playback", "Grouping", "Queue", "Favorites", "Scenes", "Spotify", "SMAPI", "Live events", "JSON / TSV"];
  const features = [
    { title: "Reliable discovery", body: "SSDP + ZoneGroupTopology + subnet-scan fallback. Names match the Sonos app." },
    { title: "Coordinator-aware", body: "Targets any room; transport commands automatically go to the group coordinator." },
    { title: "Spotify, no creds", body: "Open Spotify URIs and search via SMAPI using the service you already linked in Sonos." },
    { title: "Scenes & grouping", body: "Snapshot grouping plus per-room volume / mute, then re-apply later." },
    { title: "Live events", body: "Subscribe to AVTransport + RenderingControl and stream changes as they arrive." },
    { title: "Scriptable", body: "plain | json | tsv output, predictable exit codes, and a --debug SOAP trace." },
  ];
  return `<header class="home-hero">
        <p class="eyebrow">Sonos · One CLI</p>
        ...
        <div class="home-services" aria-label="What's in the binary">
          ${services.map((s) => `<span>${escapeHtml(s)}</span>`).join("")}
        </div>
        <p class="muted"><a href="${installRel}">Other install options →</a></p>
        <div class="home-features">
          ${features.map((f) => `<div class="home-feature"><div class="icon" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div><h3>${escapeHtml(f.title)}</h3><p>${escapeHtml(f.body)}</p></div>`).join("")}
        </div>
      </header>`;
}
    ...
    <aside class="sidebar">
      <div class="sidebar-head">
        <a class="brand" href="${hrefToOutRel("index.html", page.outRel)}" aria-label="${productName} docs home">
          ...
          <span><strong>${escapeHtml(productName)}</strong><small>Sonos · CLI docs</small></span>
        </a>
        ${themeToggleHtml()}
      </div>
      <label class="search"><span>Search</span><input id="doc-search" type="search" placeholder="play, queue, smapi…"></label>
...
function navHtml(currentPage) {
  return nav
    .map((section) => {
      const dataAttr = proseSections.has(section.name) ? ' data-section="prose"' : "";
      return `<section${dataAttr}><h2>${escapeHtml(section.name)}</h2>${section.pages.map((page) => {
        const href = hrefToOutRel(page.outRel, currentPage.outRel);
        const active = page.rel === currentPage.rel ? " active" : "";
        return `<a class="nav-link${active}" href="${href}">${escapeHtml(navTitle(page, section.name))}</a>`;
      }).join("")}</section>`;
    })
    .join("");
}
